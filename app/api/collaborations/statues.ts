...
export async function loader() {
    try {
        const statuses = await prisma.collaborationStatus.findMany({
...
const createStatus = async (request: Request) => {
    try {
        ...
        const status = await prisma.collaborationStatus.create({
            data: {
                name: parsed.name,
        ...
        console.error("Create col status failed:", error)
